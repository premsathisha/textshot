    ...
    os_activity_initiate("-[SRRecorderControlStyleResourceLoader infoForStyle:]", OS_ACTIVITY_FLAG_DEFAULT, (^{
        ...
        @synchronized (self)
        {
            ...
            if (!info)
            {
                ...
                if (!data)
                {
                    NSBundle *resourceBundle = SRBundle();
                    NSString *relativeDatasetPath = [NSString stringWithFormat:@"Images.xcassets/%@.dataset/info.json", resourceName];
                    NSArray<NSString *> *candidatePaths = @[
                        [[resourceBundle resourcePath] stringByAppendingPathComponent:relativeDatasetPath],
                        [[NSBundle.mainBundle resourcePath] stringByAppendingPathComponent:relativeDatasetPath]
                    ];
