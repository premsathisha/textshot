...
NSBundle *SRBundle()
{
    ...
    if (resourceURL)
    {
        NSURL *fallbackURL = [resourceURL URLByAppendingPathComponent:@"ShortcutRecorder_ShortcutRecorder.bundle"];
        Bundle = [NSBundle bundleWithURL:fallbackURL];
        if (Bundle)
            return Bundle;
    }

    NSURL *bundleURL = [NSBundle.mainBundle.bundleURL URLByAppendingPathComponent:@"Contents/Resources/ShortcutRecorder_ShortcutRecorder.bundle"];
    Bundle = [NSBundle bundleWithURL:bundleURL];
    if (Bundle)
        return Bundle;

    @throw [NSException exceptionWithName:NSInternalInconsistencyException
                                   reason:@"Unable to find SwiftPM ShortcutRecorder resource bundle."
                                 userInfo:nil];
#else
